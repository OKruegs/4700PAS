...
% Defining material properties for silicon
EpiSi = C.eps_0*11.68;
MunSi = 1400*1e-4;              % cm2 V-1s-1 * 1/(100 cm/m)^2 Electron mobility in silicon
DnSi = MunSi*C.kb*Temp/C.q_0;   % D = kt/q Mun Electron diffusion coefficient
MupSi = 450*1e-4;               % cm2 V-1s-1 * 1/(100 cm/m)^2 Hole mobility in silicon
DpSi = MunSi*C.kb*Temp/C.q_0;   % D = kt/q Mun Hole diffusion coefficient
tauSi = 1e-8;                   % Carrier lifetime in silicon
...
% Loading simulation parameters based on selected simulation type
...
elseif strcmp(Simulation,'NewSim')
    eval('SetNewSim');
elseif strcmp(Simulation,'NewPN')
    eval('SetNewPN');
end
...
% Initializing carrier properties
Mun = ones(1,nx)*MunSi;
Dn = ones(1,nx)*DnSi;
...
% Calculating initial carrier concentrations
if TwoCarriers == 1
    ni = NetDoping >= 0;
...
% Initializing divergence of electric flux densities
divFn = zeros(1,nx);
divFp = zeros(1,nx);

% Initializing charge density
Rho = zeros(1,nx);
if (Coupled)
